...
class EventFactory {
    ...
    _parsePayload(payload) {
        if (this._isObjectError(payload)) {
            ...
            payload.stack = parse(payload.stack);
    ...
    _isErrorEvent(eventName) {
        return startsWith(eventName, 'ERR');
    }

    _isWarningEvent(eventName) {
        return startsWith(eventName, 'WARN');
    }

    _clearIrrelevantFields(event) {
        return pickBy(event, identity);
    }
